...
router.post('/sendData', (request, response) => {
    temperatura = ArduinoDataTemp.List[ArduinoDataTemp.List.length -1];
    ...
    chave = ArduinoDataSwitch.List[ArduinoDataSwitch.List.length - 1];

    var sql = "INSERT INTO medidas (temperatura, luminosidade, umidade, chave) VALUES(?, ?, ?, ?)";

    db.query(sql,[temperatura,luminosidade, umidade, chave],function(err, result) {
        if (err) throw err;
