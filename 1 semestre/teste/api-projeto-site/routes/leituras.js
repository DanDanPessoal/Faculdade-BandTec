...
router.get('/cliente/:nick_player', function(req, res, next) {
	console.log('Recuperando jogadores');
	
	//var idcaminhao = req.body.idcaminhao; // depois de .body, use o nome (name) do campo em seu formulário de login
	var nick_player = req.params.nick_player;
	
	let instrucaoSql = "";
	
	if (env == 'dev') {
		// abaixo, escreva o select de dados para o Workbench
		instrucaoSql = `select nick_lol, rota_preferida from usuario where nick_lol = ${nick_player}`;
	} else if (env == 'production') {
		// abaixo, escreva o select de dados para o SQL Server
